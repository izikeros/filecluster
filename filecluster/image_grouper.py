...
def update_cluster_info(delta_from_previous, max_time_delta, index,
                        new_cluster_idx, list_new_clusters, media_date, cluster):
    ...
    :param delta_from_previous: distance of this image to previous (on image list sorted by time)
    :param max_time_delta: max allowed time delta to consider images as belonging to the same event
    :param index: int - image index from "inbox" media dataframe. Used only to check if we are starting clustering.
    :param new_cluster_idx: new cluster id in case we need to create new cluster
    :param list_new_clusters: list of new cluster dictionaries
    :param start_date: current "buffer" start date
    :param end_date: current "buffer" end date
    :param cluster: cluster dictionary with description of current "buffer" cluster
    :return:
    """
    # TODO: KS: 2020-12-09: introduce custom types to verify input types
    # TODO: KS: 2020-12-09: consider using TypedDict as cluster (won't work on older versions of python)

    # check if new cluster encountered

    is_first_image_analysed = index == 0
    is_this_image_too_far_from_other_in_the_cluster = delta_from_previous > max_time_delta
    if is_this_image_too_far_from_other_in_the_cluster or is_first_image_analysed:
        new_cluster_idx += 1  # FIXME: KS: 2020-12-09: should'n this be incremented only in case of first image?
...
class ImageGrouper(object):
    def __init__(self,
        ...
        :param media_df: media database (loaded from file or empty)
        :param df_clusters: clusters database (loaded from file or empty)
        :param inbox_media_df: dataframe with inbox media
        """
    ...
    def calculate_gaps(self, date_col, delta_col):
        ...
        self.inbox_media_df.sort_values(by=date_col,
                                      ascending=True,
                                      inplace=True)
    ...
    def assign_images_to_existing_clusters(self, assign_to_existing_is_active=False):
        ...
        :param assign_to_existing_is_active: enable assigning to existing clusters
        :return:
        """
    ...
    def add_cluster_id_to_files_in_data_frame(self):
        ...
        for media_index, _row in self.inbox_media_df[sel_not_clustered].iterrows():
            ...
            cluster_dict, new_cluster_idx, list_new_cluster_dictionaries = update_cluster_info(
                delta_from_previous, max_time_delta, media_index, new_cluster_idx,
                list_new_cluster_dictionaries, media_date, cluster_dict)
